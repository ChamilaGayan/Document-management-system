...
<?php
          ...
          if($type == 'leave')
          {
          $query = $db->query("SELECT * FROM images WHERE isRead = 1 ORDER BY id DESC");
          $tit="Leave Details";
          }else if($type == 'efuel')
          {
            $query = $db->query("SELECT * FROM extra_fuel WHERE isRead = 1 ORDER BY id DESC");
            $tit="Extra Fuel Details";
          }
          else if($type == 'wworks')

          {
            $query = $db->query("SELECT * FROM weekend_works WHERE isRead = 1 ORDER BY id DESC");
            $tit="Holiday Pay Details";
          }
          else if($type == 'pperfromed')

          {
            $query = $db->query("SELECT * FROM program_perfomed WHERE isRead = 1 ORDER BY id DESC");
            $tit="Work Done Details";
          }
          else if($type == 'wexpected')

          {
            $query = $db->query("SELECT * FROM work_expected WHERE isRead = 1 ORDER BY id DESC");
            $tit="Advanced Program Details";
          }
          else if($type == 'ot')

          {
            $query = $db->query("SELECT * FROM ot WHERE isRead = 1 ORDER BY id DESC");
...
<html lang="en">
  ...
  <!-- Database View -->
  <input type="text" id="myInput" onkeyup="myFunction()" placeholder="Search Divisional Secretariat Here" title="Search">
...
$stat=$row["stat"];
   ...
   echo "<a href='preview.php?type=$type&&reqId=".$row["id"]."'> View </a> </center> </td>
    </tr> ";        
  }
}
else{ 
