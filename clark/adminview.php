...
<?php
          ...
          if($type == 'leave')
          {
          $query = $db->query("SELECT * FROM images WHERE isRead = 0 OR isRead = 5");
          $tit="Leave Details";
          }else if($type == 'efuel')
          {
            $query = $db->query("SELECT * FROM extra_fuel WHERE isRead = 0 OR isRead = 5");
            $tit="Extra Fuel Details";
          }
          else if($type == 'wworks')

          {
            $query = $db->query("SELECT * FROM weekend_works WHERE isRead = 0 OR isRead = 5");
            $tit="Holiday Pay Details";
          }
          else if($type == 'pperfromed')

          {
            $query = $db->query("SELECT * FROM program_perfomed WHERE isRead = 0 OR isRead = 5");
            $tit="Work Done Details";
          }
          else if($type == 'wexpected')

          {
            $query = $db->query("SELECT * FROM work_expected WHERE isRead = 0 OR isRead = 5");
            $tit="Advanced Program Details";
          }
          else if($type == 'ot')

          {
            $query = $db->query("SELECT * FROM ot WHERE isRead = 0 OR isRead = 5");
...
if($query->num_rows>0){
  while($row=$query->fetch_assoc()){
   ...
   $row["user_id"];
    ...
    <td>". $row["rcrt"]. "</td>
    <td> <center> <a href='submit/index.php?type=$type&&reqId=".$row["id"]."&&usid=".$row["user_id"]."'> View </a> </center> </td>
    </tr>";           
  }
}
else{ 
